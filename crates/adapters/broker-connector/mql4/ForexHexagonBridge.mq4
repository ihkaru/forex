...
input int    InpHistoryLimit = 300;         // Jumlah candle H1 yang dikirim (Cukup untuk Pola N)
...
int OnInit()
{
   Print("🚀 Menginisialisasi ForexHexagonBridge ke Rust Quant Server (Port: ", InpPort, ")...");
...
void OnTimer()
{
   if(!is_connected) {
      ...
      // Kirim live tick heartbeat dan lilin berjalan
      SendCurrentTick();
      SendCurrentBar();
   }
}
...
void SendCurrentBar()
{
   ...
   double bar_close  = iClose(_Symbol, PERIOD_H1, 0);
   long   bar_vol    = iVolume(_Symbol, PERIOD_H1, 0);
...
void SendHistoricalH1Bars()
{
   ...
   Print("✅ Sukses sinkronisasi ", bars_to_send, " bar H1 ke Rust Engine via TCP socket.");
}
...
void SendSocketData(string msg)
{
   ...
   int res = send(socket_handle, buf, ArraySize(buf) - 1, 0);
   if(res < 0) {
      is_connected = false;
      historical_sent = false;
      closesocket(socket_handle);
